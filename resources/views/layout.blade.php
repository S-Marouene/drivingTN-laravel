...
<body>
<nav class="navbar navbar-expand-lg navbar-dark"><div class="container"><a class="navbar-brand fw-bold" href="{{ route('home') }}"><i class="bi bi-sign-turn-right-fill brand-dot"></i> Permis TN</a><div class="ms-auto d-flex gap-2"><a class="btn btn-sm btn-outline-light" href="{{ route('home') }}">Accueil</a><a class="btn btn-sm btn-warning" href="{{ route('admin.index') }}"><i class="bi bi-shield-lock"></i> Administration</a></div></div></nav>
<main>@if(session('success'))<div class="container mt-3"><div class="alert alert-success alert-dismissible fade show">{{ session('success') }}<button class="btn-close" data-bs-dismiss="alert"></button></div></div>@endif @if($errors->any())<div class="container mt-3"><div class="alert alert-danger"><strong>Veuillez corriger :</strong><ul class="mb-0">@foreach($errors->all() as $error)<li>{{ $error }}</li>@endforeach</ul></div></div>@endif @yield('content')</main>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>@yield('scripts')
</body></html>
